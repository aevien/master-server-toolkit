        ...
        private async Task GetArtifactsMessageHandler(IIncomingMessage message)
        {
            ...
            if (json.HasField("data") && json["data"].IsArray)
            {
                ...
                foreach (var item in json["data"])
                {
                    ...
                    product.Price = $"{product.PriceValue} {product.PriceCurrencyCode}";
                    product.Extra.AddField("game_id", item["game_id"].IntValue);
                    product.Extra.AddField("kind", item["kind"].StringValue);
